...
const page = () => {
  ...
  const filteredSpaces = selectedFilter === "All" 
    ...
    : spacesData.filter(space => 
        space.categories && space.categories.includes(selectedFilter)
      );
        ...
        <section className="spaces-header">
          <div className="container">
            ...
            <div className="prop-col">
              <Copy delay={1}>
                <h1>Influence In Action</h1>
              </Copy>
              <div className="prop-filters">
                <div 
                  className={`filter ${selectedFilter === "All" ? "default active" : ""}`}
                ...
                <div 
                  className={`filter ${selectedFilter === "Branding" ? "active" : ""}`}
                  onClick={() => handleFilterClick("Branding")}
                >
                  <Copy delay={1.1}>
                    <p className="lg">Branding</p>
                  </Copy>
                </div>
                <div 
                  className={`filter ${selectedFilter === "Fundraising" ? "active" : ""}`}
                  onClick={() => handleFilterClick("Fundraising")}
                >
                  <Copy delay={1.2}>
                    <p className="lg">Fundraising</p>
                  </Copy>
                </div>
                <div 
                  className={`filter ${selectedFilter === "Storytelling" ? "active" : ""}`}
                  onClick={() => handleFilterClick("Storytelling")}
                >
                  <Copy delay={1.3}>
                    <p className="lg">Storytelling</p>
                  </Copy>
                </div>
              </div>
            </div>
